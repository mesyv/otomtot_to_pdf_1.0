...
try:
    dictionary = open("slownik.json", encoding='utf-8')
    translation = json.load(dictionary)
except:
    print("Brak pliku \"slownik.json\" w lokalizacji pliku .exe")
...
def create_pdf():
    ...
    if not url:
        ...
        equip0 = equipment[0].splitlines()
        equip1 = equipment[1].splitlines()
        equip2 = equipment[2].splitlines()

        for i in range(len(equip0)):
            try:
                equip0[i] = translation[equip0[i]]
            except:
                print(f"W slowniku brak tlumaczenia: {equip0[i]}")
                equip0[i] = ""

        for i in range(len(equip1)):
            try:
                equip1[i] = translation[equip1[i]]
            except:
                print(f"W slowniku brak tlumaczenia: {equip1[i]}")
                equip1[i] = ""

        for i in range(len(equip2)):
            try:
                equip2[i] = translation[equip2[i]]
            except:
                print(f"W slowniku brak tlumaczenia: {equip2[i]}")
                equip2[i] = ""

        equip0 = list(filter(None, equip0))
        equip1 = list(filter(None, equip1))
        equip2 = list(filter(None, equip2))
        ...
        for a, b in details_dict.items():
            if b != "N/a":
                if l == 4 or l == 8 or l == 12 or l == 16 or l == 20 or l == 24 or l ==28:
                    try:
                        pdf.cell(WIDTH/4, 6, a + " " + translation[b], border=bord, align="C", ln=True)
                    except:
                        pdf.cell(WIDTH/4, 6, a + " " + b, border=bord, align="C", ln=True)
                else:
                    try:
                        pdf.cell(WIDTH/4, 6, a + " " + translation[b], border=bord, align="C")
                    except:
